...
const getCodigoPostal = async (req, res) => {
    ...
    try {
      ...
      const [colonias, municipio, localidad] = await Promise.all([
        ...
        Municipio.findOne({ c_Municipio: codigoPostalEncontrado.c_Municipio }),
        Localidad.findOne({ c_Localidad: codigoPostalEncontrado.c_Localidad }),
      ]);
      return res.status(200).json({
        status: 'Ok',
        message: 'Código postal encontrado',
        data: {
          c_CodigoPostal: codigoPostalEncontrado.c_CodigoPostal,
          c_Estado: codigoPostalEncontrado.c_Estado,
          c_Municipio: codigoPostalEncontrado.c_Municipio,
          c_Localidad: codigoPostalEncontrado.c_Localidad,
          EstimuloFranjaFronteriza: codigoPostalEncontrado.EstimuloFranjaFronteriza,
          id: codigoPostalEncontrado.id,
          c_PaisNombre,
          c_EstadoNombre,
          colonias, municipio, localidad
        },
      });
  } catch (error) {
