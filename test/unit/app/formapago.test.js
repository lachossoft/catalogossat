...
describe("Prueba que devolvera un status 200 para indicar que las peticiones para el controlador de las formas de pago respondieron correctamente", () => {
  test("Validar que traiga los valores completos y que no sea nulo", async () => {
    const formapagolist = await api.get("/api/catFormaPago/list");
  ...
  test("Validar que traiga los datos correctos", async () => {
    ...
    expect(oneformapago.statusCode).toBe(200);
    expect(oneformapago.body.message).toEqual(
      "Se encontró la forma de pago solicitada"
    );
